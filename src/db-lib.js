"use strict";
/**
 * SQLite implementation for Signifly Case Assignment for Paul Kirkaas
 * Paul Kirkaas, March 2023
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.tableExists = exports.createTbl = exports.openDb = void 0;
var init_js_1 = require("./init.js");
var sqlite3_1 = require("sqlite3");
var sqlite_1 = require("sqlite");
sqlite3_1.default.verbose();
;
// you would have to import / invoke this in another file
function openDb(filename) {
    if (filename === void 0) { filename = './tmp/signifly.db'; }
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, (0, sqlite_1.open)({
                    filename: filename,
                    driver: sqlite3_1.default.Database
                })];
        });
    });
}
exports.openDb = openDb;
/**
 * create table in DB if not exists - create id by default
 */
function createTbl(db, tblName, colDefs) {
    return __awaiter(this, void 0, void 0, function () {
        var colDefArr, colDefStr, cstr;
        return __generator(this, function (_a) {
            if (!('id' in colDefs)) {
                //colDefs.id = "int primary key not null AUTOINCREMENT";
                colDefs.id = "INTEGER PRIMARY KEY AUTOINCREMENT";
            }
            colDefArr = Object.entries(colDefs).map(function (entry) {
                return entry.join(' ');
            });
            colDefStr = colDefArr.join(', ');
            cstr = "CREATE TABLE IF NOT EXISTS '".concat(tblName, "' (").concat(colDefStr, ");");
            console.log({ cstr: cstr });
            return [2 /*return*/, db.exec(cstr)];
        });
    });
}
exports.createTbl = createTbl;
function tableExists(db, tblName) {
    return __awaiter(this, void 0, void 0, function () {
        var teStr, row, vals;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    teStr = " SELECT EXISTS ( SELECT 'name' FROM 'sqlite_schema' WHERE \n        type='table' AND name='".concat(tblName, "'\n    );");
                    return [4 /*yield*/, db.get(teStr)];
                case 1:
                    row = _a.sent();
                    if (!row || (0, init_js_1.isEmpty)(row) || !(0, init_js_1.isObject)(row)) {
                        return [2 /*return*/, false];
                    }
                    vals = Object.values(row);
                    if (!Array.isArray(vals) || !vals.length) {
                        return [2 /*return*/, false];
                    }
                    return [2 /*return*/, vals[0]];
            }
        });
    });
}
exports.tableExists = tableExists;
